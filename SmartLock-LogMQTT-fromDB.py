...
PUB_TOPIC = "InouelabSmartHomes/SmartLock/KeyLog"
      ...
      if target_res != None:
        ...
        for registered_idm in registered_idms:
          if registered_idm in visitor_idm:
            ...
            pub_Message = '{"TimeStamp" : "' + datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S") + '", "state" : "' + state + '", "SIT_ID" : "' + sit_id + '", "type" : "' + device + '"}'
            print(pub_Message)
        ...
        if registered_flag == False:
          ...
          if registered_flag == False:
            pub_Message = '{"TimeStamp" : "' + datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S") + '", "state" : "' + visitor_idm + '", "SIT_ID" : "Unknown", "type" : "' + str(visitor_idm) + '"}'
            print(pub_Message)
            mqtt_publisher.publish(PUB_TOPIC, pub_Message, qos=0)
            print("This is unknown IC card")
